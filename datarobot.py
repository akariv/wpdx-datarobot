...
def do_predictions(country_codes, training_path, prediction_path, validation_path, base_path, years_range):
    ...
    for country_code in ['all'] + country_codes:
        ...
        if len(existing_projects) > 0:
            ...
            if not reuse:
                print('FOUND EXISTING PROJECT, DELETING: {}'.format(project.project_name))
                project.delete()
                project = None
            else:
    ...
    for country_code in country_codes:
        ...
        outdir = TMPDIR / country_code
        validation_file = f'data/{country_code}_water_points.csv'
        ...
        for model_idx in ['all', country_code]:
            ...
            validation_dataset = project.upload_dataset(str(validation_dir / validation_file), max_wait=TIMEOUT)
